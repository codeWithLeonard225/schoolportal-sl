...
// 🛑 REMOVED: const CLASS_OPTIONS = [...] 
// Classes will now be fetched dynamically from Firestore
...
const Registration = () => {
    ...
    const [allClasses, setAllClasses] = useState([]); // 🌟 NEW: State for dynamic classes
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            toast.error("Failed to stream real-time student data.");
        });
    ...
    // 🌟 REAL-TIME LISTENER for Classes (Classes collection)
    useEffect(() => {
        const classCollectionRef = collection(db, "Classes");
        // Optional: Order the classes by className
        const q = query(classCollectionRef); 

        const unsubscribeClasses = onSnapshot(q, (snapshot) => {
            // Map to just the class names for the dropdown, and sort them
            const classNames = snapshot.docs.map(doc => doc.data().className);
            setAllClasses(classNames.sort()); 
        }, (error) => {
            console.error("Failed to stream classes:", error);
    ...
    // 🔎 FILTER LOGIC using useMemo for efficiency (No changes needed)
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow-lg rounded-2xl p-6 w-full max-w-2xl">
                ...
                <div className="flex flex-col md:flex-row md:space-x-4">
                    <div className="flex-1">
                        ...
                        <select name="class" value={formData.class} onChange={handleInputChange} className="w-full p-2 mb-4 border rounded-lg" required>
                            <option value="">Select Class</option>
                            {/* 🌟 UPDATED: Mapping over dynamically fetched allClasses */}
                            {allClasses.map(c => (
                    ...
                    <div className="flex-1">
                        ...
                        <select name="academicYear" value={formData.academicYear} onChange={handleInputChange} className="w-full p-2 mb-4 border rounded-lg" required>
