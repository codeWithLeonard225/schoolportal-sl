...
const TermResult = () => {
    ...
    // 🔹 2B. Fetch Classes Cache (Necessary for correct Percentage divisor)
    useEffect(() => {
        if (!schoolId) return;
    ...
    // --- 3. CALCULATIONS (UPDATED PERCENTAGE LOGIC) ---
    const broadSheetData = useMemo(() => {
        ...
        // 🔹 Determine the divisor once for the class
        ...
        pupils.forEach(pupil => {
            ...
            summaries[pupil.studentID] = {
                ...
                percentage: totalSubjectPercentage > 0 ? ((ov.total / totalSubjectPercentage) * 100).toFixed(1) : 0,
                rank: ov.pos
            };
        });
  ...
  doc.save(`${selectedClass}_BroadSheet_${selectedTerm}.pdf`);
    ...
    return (
        <div className="max-w-full mx-auto p-4 bg-white shadow-xl rounded-2xl">
            {/* Header and Selectors remain same */}
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                <h2 className="text-xl font-bold text-indigo-700">{schoolName} Broad Sheet</h2>
                <button onClick={handlePrint} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg shadow-md transition-all font-semibold">Print Broad Sheet</button>
            </div>

            {/* Main Table View */}
            {loading ? (
                <div className="text-center p-10 text-indigo-600 font-bold">Fetching records...</div>
            ) : (
                <div className="overflow-x-auto border rounded-xl shadow-inner">
                    <table className="min-w-full text-[10px] text-center border-collapse">
                        {/* Table headers and rows logic using the updated broadSheetData */}
                        <thead className="bg-indigo-700 text-white sticky top-0 z-20">
                            <tr>
                                ...
                                {pupils.filter(p => selectedPupil === "all" || p.studentID === selectedPupil).map(p => (
                                    <th key={p.studentID} colSpan="4" className="px-2 py-2 border-b border-r min-w-[140px] uppercase">{p.studentName}</th>
                                ))}
                            </tr>
                            <tr className="bg-indigo-600 text-[9px]">
                                {pupils.filter(p => selectedPupil === "all" || p.studentID === selectedPupil).map(p => (
                                    <React.Fragment key={`sub-${p.studentID}`}>
                                        <th className="px-1 py-1 border-r">T1</th><th className="px-1 py-1 border-r">T2</th><th className="px-1 py-1 border-r bg-indigo-800">Mn</th><th className="px-1 py-1 border-r text-red-200">Rnk</th>
                                    </React.Fragment>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {broadSheetData.subjects.map((sub) => (
                                <tr key={sub} className="border-b hover:bg-gray-50 bg-white">
                                    <td className="text-left px-4 py-2 font-bold border-r sticky left-0 bg-white">{sub}</td>
                                    {pupils.filter(p => selectedPupil === "all" || p.studentID === selectedPupil).map(p => {
                                        ...
                                        return (
                                            <React.Fragment key={`${p.studentID}-${sub}`}>
                                                ...
                                                <td className={`px-1 py-2 border-r ${getGradeColor(res.test2)}`}>{res.t2 || "-"}</td>
