...
const AnnualBroadSheet = () => {
  ...
  const [calcMode, setCalcMode] = useState("auto"); // "auto", "term1_2", "term2_3", "3"
  ...
  // 4. Main Calculation Engine (Upgraded with Dynamic Divisor Logic)
  const annualData = useMemo(() => {
    ...
    // Calculate Adaptive Averages for Standings Across All Students
    ...
    subjects.forEach(sub => {
      const subjectScores = studentIDs.map(id => {
        ...
        // Dynamic engine execution for context parsing
        let divisor = 3;
        ...
        return { id, yearlyMean: Math.round(scoreSum / divisor) };
    ...
    const allStudentsStats = studentIDs.map(id => {
      ...
      subjects.forEach(sub => {
        ...
        totalAnnualAccumulator += Math.round(scoreSum / divisor);
      });
  ...
  ann: subjects.length ? (activePupilStats.annual / subjects.length).toFixed(1) : "0.0"
    ...
    const rows = subjects.map(sub => {
      ...
      return { 
        ...
        ann: Math.round(scoreSum / divisor), 
        annRank: subjectYearlyRanks[sub].find(s => s.id === selectedPupil)?.pos || "-" 
      };
    });

    return { rows, footers };
  }, [classGradesData, selectedPupil, pupils, calcMode]);
  ...
  const handlePrintPDF = () => {
    ...
    Promise.all([loadImage(schoolLogoUrl), loadImage(pupilPhotoUrl)]).then(([logo, pupilPhoto]) => {
      ...
      doc.text(`Academic Year: ${academicYear} (${calcMode.toUpperCase()})`, rightBoundary, y, { align: "right" });
      y += 25;
  ...
  return (
    <div className="p-4 bg-slate-100 min-h-screen">
      <div className="max-w-[1550px] mx-auto bg-white shadow-2xl rounded-3xl p-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center mb-6 gap-4 bg-slate-900 p-6 rounded-2xl text-white">
          <h2 className="text-xl font-bold uppercase tracking-wider">Annual BroadSheet Portal</h2>
          <div className="flex flex-wrap gap-3 items-center w-full lg:w-auto justify-end">
            
            {/* Calculation Mode Selector Dropdown */}
            <select 
              className="bg-red-50 text-red-900 p-2 rounded-lg font-bold text-sm border-2 border-red-200 focus:outline-none" 
              value={calcMode} 
              onChange={e => setCalcMode(e.target.value)}
            >
              <option value="auto">🔄 Auto Detect (Active Terms)</option>
              <option value="term1_2">🔢 Force Divide by 2 (Terms 1 & 2)</option>
              <option value="term2_3">🔢 Force Divide by 2 (Terms 2 & 3)</option>
              <option value="3">🔢 Force Divide by 3 (Full Year)</option>
            </select>

            <select className="text-black p-2 rounded-lg text-sm font-medium" value={academicYear} onChange={e => setAcademicYear(e.target.value)}>
              {academicYears.map(y => <option key={y} value={y}>{y}</option>)}
            </select>
            <select className="text-black p-2 rounded-lg text-sm font-medium" value={selectedClass} onChange={e => setSelectedClass(e.target.value)}>
              {availableClasses.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <select className="text-black p-2 rounded-lg font-bold text-sm" value={selectedPupil} onChange={e => setSelectedPupil(e.target.value)}>
