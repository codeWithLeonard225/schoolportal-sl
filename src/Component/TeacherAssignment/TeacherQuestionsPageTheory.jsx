...
// 💾 Initialize localforage stores (Re-using the existing instances)
...
const TeacherQuestionsPageTheory = () => {
    ...
    // Structure for a Theory Question: Only 'question' and 'topic'
    ...
    // This logic remains the same as it loads class/subject dropdown options
    useEffect(() => {
    ...
    // This logic remains the same as topic management is universal
    useEffect(() => {
    ...
    // --- 3. LOAD QUESTIONS (TheoryQuestionsBank) ---
    // **KEY DIFFERENCE**: Uses "TheoryQuestionsBank" collection
    useEffect(() => {
        ...
        const q = query(
            collection(schoollpq, "TheoryQuestionsBank"), // <--- Changed collection name
            where("schoolId", "==", schoolId),
    ...
    const resetForm = () => {
        setQuestions([
            { number: 1, topic: selectedTopic, question: "" } // Retain selected topic on reset
        ]);
    ...
    const handleEditQuestion = (questionToEdit, index) => {
        ...
        // Theory questions have no 'options' or 'answer'
        setQuestions([{ 
    ...
    const handleDeleteQuestion = async (questionToDelete, index) => {
        ...
        if (!window.confirm("Are you sure you want to permanently delete this theory question?")) return;

        try {
            const docRef = doc(schoollpq, "TheoryQuestionsBank", existingDocId); // <--- Changed collection
    ...
    const handleSubmitQuestions = async () => {
        ...
        try {
            ...
            // Get the clean object to save (remove temp fields)
            ...
            if (editingQuestionIndex !== null) {
                // --- Logic for Update (Relies on Question content matching) ---
                const questionToFind = fetchedQuestions[editingQuestionIndex]; 
                
                // Find index in the full list based on the old question data
                ...
                if (indexInAllQuestions !== -1) {
                    ...
                    // Fallback to adding new if update failed (Shouldn't happen with stable data)
            ...
            const docRef = existingDocId
                ? doc(schoollpq, "TheoryQuestionsBank", existingDocId) // <--- Changed collection
                : doc(collection(schoollpq, "TheoryQuestionsBank")); // <--- Changed collection
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 bg-white rounded-xl shadow-lg">
            ...
            {/* -------------------- 2. TOPIC MANAGER -------------------- */}
            {/* Re-using the same topic management system */}
            ...
            <textarea
                ...
                rows="5" // Increased rows for theory questions
                value={questions[0].question}
                placeholder="Enter theory question text..."
                onChange={(e) => updateQuestion(0, "question", e.target.value)}
            />
            
            {/* REMOVED: Options and Correct Answer Selection */}
            
            <div className="mt-4 flex justify-end items-center border-t pt-3">
