            ...
            {filteredFees.length > 0 ? (
              <>
                ...
                <div className="bg-indigo-50 p-4 rounded-xl shadow-inner border border-indigo-200 mt-4 text-sm">
                    <p className="font-semibold text-blue-700">Total Charged: NLE {totalCharged.toFixed(2)}</p>
                    <p className="font-semibold text-green-700">Total Paid: NLE {totalPaid.toFixed(2)}</p>
                    <p className={`font-bold text-base ${totalBalance > 0 ? 'text-red-700' : 'text-green-700'}`}>
                        Balance Due: {totalBalance > 0 ? `GHS ${totalBalance.toFixed(2)}` : "CLEARED"}
                    </p>
                </div>
              </>
            ) : (
              ...
              {filteredFees.length > 0 && (
                <tfoot>
                  <tr className="bg-indigo-100 font-extrabold text-sm border-t-2 border-indigo-500">
                    <td className="p-3 border-r text-left" colSpan="2">TOTALS FOR {selectedYear}:</td>
                    <td className="p-3 border-r text-right text-blue-800">{totalCharged.toFixed(2)}</td>
                    <td className="p-3 border-r text-right text-green-700">{totalPaid.toFixed(2)}</td>
                    <td className={`p-3 border-r text-right ${totalBalance > 0 ? 'text-red-700' : 'text-green-700'}`}>{totalBalance.toFixed(2)}</td>
